...
class KF_Dataset():
    def __init__(self, csv_path, spilt):
        super(KF_Dataset, self).__init__()
        self.spilt = spilt
        self.imgs, self.labels = load_data(csv_path)
...
def create_dataset(train_csv_path, val_csv_path, batch_size=32, image_size=224):
    train_dataset = KF_Dataset(train_csv_path, spilt="train")
    val_dataset = KF_Dataset(val_csv_path, spilt="val")
    # 定义数据增强
    ...
    train_dataset = de.GeneratorDataset(train_dataset, ["image", "label"])
    train_dataset = train_dataset.map(operations=trans_train, input_columns="image", num_parallel_workers=8)

    val_dataset = de.GeneratorDataset(val_dataset, ["image", "label"])
    val_dataset = val_dataset.map(operations=trans_val, input_columns="image", num_parallel_workers=8) 
...
if __name__ =="__main__":
    ...
    _, dataset_val = create_dataset(datacsv_path_train, datacsv_path_test, batch_size=30,image_size=512)
    print("done")
    context.set_context(mode=context.GRAPH_MODE, device_target="GPU")
    net = resnet101(6)
    print("load ckpt into resnet101........")
    from mindspore.train.serialization import load_param_into_net, load_checkpoint
    param_dict = load_checkpoint(ckpt_file_name="/data/users/user2/xjm/demo/resnet101_epoch44_acc77.1667.ckpt")
    ...
    acc4 = []
    acc5 = []
    length = len(dataset_val.create_dict_iterator(num_epochs=1))
    for item in dataset_val.create_dict_iterator(num_epochs=1):
